...
pForm.gfn_SetFirstRow = function(objDs, sCd, sName, sFirstGB){
	var gtrcPos = "Util.xjs.gfn_SetFirstRow";
	 
		this.gtrace("ㅇ 첫줄세팅(gfn_SetFirstRow)--->"+objDs.name, gtrcPos);
		this.gtrace("데이터셋명(objDs)--->"+objDs.name, gtrcPos);
		this.gtrace("코드(sCd	)--->"+sCd, gtrcPos);
		this.gtrace("이름(sName)--->"+sName, gtrcPos);
		this.gtrace("첫줄구분(sFirstGB)--->"+sFirstGB, gtrcPos);
...
pForm.gfn_dsLookup = function(ds, varCol, varCmpVal, strOutputColID){
	var fRp = ds.findRow(varCol, varCmpVal);
	return ds.getColumn(fRp, strOutputColID);
}

//	TopLevel 폼까지 거슬러 올라가면 Left와 Top의 위치를 합산
//	웹브라우져(Chrome)에서 정확한 x, y 좌표를 가져올 의도로 작성함
//	현재 : 포맷버튼의 팝업 PopupMenuGrdFormat.trackPopup 할 때 좌표를 구하기 위해 사용
pForm.gfn_getOffsetTopFromTopLevel = function(obj){
	
	var oTopFrm = this.gfn_getTopLevelForm(obj); 	//	trace("oTopFrm.name--->"+oTopFrm.name);
	var tempObj = obj;								//	trace("tempObj.name--->"+tempObj.name);
	
	//	var tempStrObj = "tempObj";
	var tempStrObj = "this";
	var vSum = obj.getOffsetTop();
	var hSum = obj.getOffsetLeft();
	
	while(oTopFrm != tempObj)
	{
		tempStrObj = tempStrObj+".parent";
		tempObj = eval(tempStrObj);					//	trace("while--tempObj.name--->"+tempObj.name);
													//	trace("tempObj.getOffsetTop--->"+tempObj.getOffsetTop());
		vSum += tempObj.getOffsetTop();
		hSum += tempObj.getOffsetLeft();
	}
